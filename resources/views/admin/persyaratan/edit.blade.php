    ...
    <div class="card-body p-4">
        <form action="{{ route('admin.persyaratan.update', $persyaratan->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nama_syarat" id="nama_syarat" class="form-control @error('nama_syarat') is-invalid @enderror" value="{{ old('nama_syarat', $persyaratan->nama_syarat) }}" required placeholder="Contoh: Sertifikat Prestasi">
                @error('nama_syarat')
            ...
            <div class="mb-3">
                ...
                <textarea name="keterangan" id="keterangan" class="form-control @error('keterangan') is-invalid @enderror" rows="3" placeholder="Jelaskan detail file yang harus diunggah...">{{ old('keterangan', $persyaratan->keterangan) }}</textarea>
                @error('keterangan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
